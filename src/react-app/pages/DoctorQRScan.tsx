...
export default function DoctorQRScan() {
  ...
  const fetchPatients = async (query = "") => {
    try {
      const url = query
        ? `/api/doctor/patients?search=${encodeURIComponent(query)}`
        : "/api/doctor/patients";
  ...
  const togglePatient = async (patientId: string) => {
    ...
    // Load QR versions for this patient if not cached
    if (!qrMap[patientId]) {
      try {
      ...
      <div className="p-6 lg:p-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-muted-foreground mt-1">View patient info with versioned QR codes</p>
          </div>
